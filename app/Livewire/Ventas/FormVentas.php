...
class FormVentas extends Component
{
    ...
    // -------------------------- Para clientes -----------------------
    public function abrirModalCliente($ventaIndex)
    {
        $this->ventaIndexCliente = $ventaIndex;
        $this->cliente_nombre = '';
        $this->cliente_documento = '';

        $this->dispatch('openModalCliente');
    }

    public function cerrarModalCliente()
    {
        $this->reset(['cliente_nombre', 'cliente_documento', 'ventaIndexCliente']);
        $this->dispatch('closeModalCliente');
    }
    ...
    public function guardarCliente()
    {
        $this->validate([
            ...
            'cliente_documento' => 'required|string|max:50|unique:clientes,documento',
        ], [
            'cliente_nombre.required' => 'El nombre es obligatorio.',
            'cliente_documento.required' => 'El documento es obligatorio.',
            'cliente_documento.unique' => 'Ya existe un cliente con ese documento.',
        ]);
        ...
        if ($this->ventaIndexCliente !== null) {
            $this->deudorIds[$this->ventaIndexCliente] = $cliente->id;
        }
    ...
    public function cargarVentasTemporales()
    {
        ...
        foreach ($ventasTemporales as $ventaIndex => $venta) {
            ...
            $creditos = DB::table('credito_temporal')
                ->where('venta_temporal_id', $venta->id)
                ->join('clientes', 'credito_temporal.deudor_id', '=', 'clientes.id') // Unimos con la tabla de usuarios
                ->select('credito_temporal.monto', 'clientes.nombre as deudor_nombre') // Obtenemos el nombre del deudor
                ->get();
