                        ...
                        <div class="col-md-4">
                            <x-select model="$wire.cuenta_id" id="cuenta_id" label="Método de pago" required="true">
                                ...
                                @foreach ($cuentas as $i)
                                    <option value="{{ $i->id }}">{{ $i->nombre }} {{ $i->numero_de_cuenta ? ' - ' . $i->numero_de_cuenta  : ''}}</option>
                                @endforeach
                            </x-select>
                        </div>
                        <div class="col-md-12 mb-2">
                                    ...
                                    <tbody>
                                        <template x-for="( item , key) in $wire.detalles" :key="key">
                                            <tr>
                                                ...
                                                <td>
                                                    <div class="d-flex">
                                                        <div class="">
                                                            ...
                                                            <template x-if="( typeof item.error_cantidad && item.error_cantidad )">
                                                                <span class="italic_sub c_red">la cantidad no puede ser 0</span>
                                                            </template>
                                                            <template x-if="( typeof item.error_vendidos && item.error_vendidos )">
                                                                <span class="italic_sub c_red" x-text="'La cantidad no puede ser inferior a ' + item.vendidos"></span>
                                                            </template>
                                                            <div class="italic_sub c_orange" x-show="item.vendidos > 0" x-text="'Vendidos: ' + item.vendidos"></div>
                                                        </div>
                                                        <div class="ml_5 d-block">
                                                            <div class="">
                                                                <a type="buttom" x-on:click="item.cantidad++"><i class="fa-solid fa-caret-up fs-22"></i></a>
                                                            </div>
                                                            <div class="">
                                                                <a type="buttom" x-on:click="if( item.cantidad > 0 ) item.cantidad-- "><i class="fa-solid fa-caret-down fs-22"></i></a>
                                                            </div>
                                                ...
                                                <td>
                                                    <x-input model="item.precio_compra" dataMask="$ #.##0,00"></x-input>
                                                    <template x-if="( typeof item.error_precio_compra && item.error_precio_compra )">
                                                        <span class="italic_sub c_red">la precio de compra no puede ser 0</span>
                                                    </template>
                                                </td>
                                                <td x-text="__numberFormat( __limpiarNum( item.precio_compra ) * item.cantidad )"></td>
                                                <td>
                                                    <template x-if="item.vendidos == 0">
                                                        <x-buttonsm  click="removeProducto( key )" color="danger"><i class="la la-trash"></i> </x-buttonsm>
                                                    </template>
                                                    <template x-if="item.vendidos > 0">
                                                        <x-buttonsm  click="alertMessage('Lo sentimos', 'No puede eliminar un producto que ya ha tenido ventas', 'error')" color="danger"><i class="la la-trash"></i> </x-buttonsm>
                                                    </template>
                                                </td>
                                            </tr>
                                        </template>
            ...
            Alpine.data('formcompras', () => ({
                ...
                get totalCompra(){
                    ...
                    @this.detalles.map( ( item ) => {
                        total += (__limpiarNum( item.precio_compra ) * item.cantidad)
                    })
                ...
                async guardar(){
                    if ( @this.detalles.length > 0 ) {
                        ...
                        @this.detalles.map( ( item, key ) => {
                            ...
                            }else if( item.cantidad < item.vendidos ){ // que la cantidad no sea inferior a lo que ya vendi
                                continuar           = false
