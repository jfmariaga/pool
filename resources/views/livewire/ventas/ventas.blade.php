                ...
                <div class="col-md-12 p-2">
                    ...
                    <div class="d-flex justify-content-between mt-2">
                        ...
                        <div><strong>Vendedor:</strong>
                            <span x-text="(comprobante.usuario ? `${comprobante.usuario.name || ''} ${comprobante.usuario.last_name || ''}` : 'N/A')"></span>
                        </div>
                    </div>
                    <hr>
                        ...
                        <tbody>
                            <template x-for="(item, key) in comprobante.det_ventas || []" :key="key">
                                <tr>
                                    ...
                                    <td x-text="comprobante.venta_mayorista ? __numberFormat(item.producto?.precio_mayorista || 0) : __numberFormat(item.precio_venta || 0)"></td>
                                    <td x-text="item.cant || '0'"></td>
                                    <td x-text="comprobante.venta_mayorista ? __numberFormat((item.producto?.precio_mayorista || 0) * (item.cant || 0)) : __numberFormat((item.precio_venta || 0) * (item.cant || 0))"></td>
                                </tr>
                            </template>
                        </tbody>
                    </table>
                    ...
                    <div class="text-right">
                        <p><strong>Subtotal:</strong> <span x-text="__numberFormat(comprobante.monto_total || 0)"></span></p>
                        <p><strong>IVA 19%:</strong> <span x-text="__numberFormat(comprobante.iva || 0)"></span></p>
                        <p><strong>Total:</strong> <span x-text="__numberFormat(comprobante.monto_total || 0)"></span></p>
                        <p><strong>Tipo de Pago:</strong> <span x-text="comprobante.cuenta?.nombre || 'N/A'"></span></p>
                    </div>
                </div>
            </template>
        </div>
    ...
    <script>
        ...
        Alpine.data('dataalpine', () => ({
            ...
            async addItem(i) {
                ...
                let metodosPagoText = '';
                if (i.metodosPago && i.metodosPago.length > 0) {
                    metodosPagoText = i.metodosPago.map(metodo => `${metodo.nombre}: ${__numberFormat(metodo.monto)}`).join('<br>');
                } else {
                    metodosPagoText = `${i.cuenta ? i.cuenta.nombre : 'N/A'}: ${__numberFormat(i.monto_total)}`;
                }
